...
public class CuckooHash<K, V> {
    ...
    public int mapSize() {
        return CAPACITY; }    // used in external testing only
    ...
    public void put(K key, V value) {
        ...
        //hash positions for h1 and h2
        int h1Thing = hash1(key);
        int h2Thing = hash2(key);

        // check if this key-value pair already exists by first making sure the bucket at h1 or h2 is not null
        // so we can  call getBucKey() and getValue() without causing a NullPointerException.
        // then check whether the bucket at h1 actually contains the same key and value as the one we're trying to insert.
        // do the same check for the bucket at h2
        if ((table[h1Thing] != null && table[h1Thing].getBucKey().equals(key) && table[h1Thing].getValue().equals(value)) ||
            ...
            //if the condition is met then we are done.
            return;
        }

        //if not then we need to go to plan b

        // start by copying the key and value into new variables, since we might need to kick values out
        // and replace them during the loop, and we don’t want to lose the original key and value
        K currKeyPoint = key;
        V currValPoint = value;

        // count will help us detect a cycle by keeping track of how many times we’ve moved items
        int count = 0;

        // we try placing the key at its first hash position to begin with
        int position = hash1(currKeyPoint);

        // start the loop that tries to insert the item
        while (count < CAPACITY) {

            // if the current spot is empty, we can place the item there and we’re done
            if (table[position] == null) {
                table[position] = new Bucket<>(currKeyPoint, currValPoint);
                return;
            }

            // if the spot is full, we need to kick out whatever is there to make room
            Bucket<K, V> kicked = table[position];

            // place the current item into the current spot
            table[position] = new Bucket<>(currKeyPoint, currValPoint);

            // now update currKey and currVal to be the item we just kicked out
            currKeyPoint = kicked.getBucKey();
            currValPoint = kicked.getValue();

            // figure out the alternate spot for the kicked-out item
            // if we just inserted into hash1, go to hash2 next. if we were at hash2, go back to hash1
            ...
            // add to the count to keep track of how many moves we’ve made
            count++;
        }

        // if we reach this point, it means we’ve hit the limit and probably entered a cycle
        // so we rehash (grow the table and re-insert everything), and then try inserting the last kicked-out item again
        rehash();
